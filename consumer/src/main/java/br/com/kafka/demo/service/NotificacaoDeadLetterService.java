...
public class NotificacaoDeadLetterService {
    ...
    public void enviarDLT(Exception exception, PayloadDTO payloadDTO){

        var rootCause = ExceptionUtils.getRootCauseMessage(exception);
        var sstackTrace = Optional.ofNullable(exception.getCause()).map(Throwable::toString).orElse("");
        ...
        log.error("### KafkaErrorHandler enviarDLT: {} message: {}", rootCause, message);
